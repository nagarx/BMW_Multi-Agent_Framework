#!/usr/bin/env python3
"""
Improved example demonstrating the Ollama-specific ReAct strategy.
"""
...
from bmw_agents.core.prompt_strategies.ollama_react import OllamaReActPromptStrategy
...
async def main() -> None:
    """Run an improved ReAct example with Ollama-specific strategy."""
    ...
    # Create the Ollama-specific ReAct prompt strategy
    prompt_strategy = OllamaReActPromptStrategy(
        llm_provider=llm_provider, tools=tools, max_iterations=5
    )

    # Check initial state
    print(
        "Initial - Thoughts: {}, Actions: {}, Observations: {}".format(
            len(prompt_strategy.thoughts),
            len(prompt_strategy.actions),
            len(prompt_strategy.observations),
        )
    )

    # Run the strategy with a user query
    user_query = (
        "What is the current time? Then calculate the sum of 123.45 and 678.9, "
        "and finally multiply the result by a random number between 1 and 10."
    )
    ...
    # Check final state
    print(
        "After run - Thoughts: {}, Actions: {}, Observations: {}".format(
            len(prompt_strategy.thoughts),
            len(prompt_strategy.actions),
            len(prompt_strategy.observations),
        )
