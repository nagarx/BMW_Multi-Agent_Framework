#!/usr/bin/env python3
"""
Improved example demonstrating the Ollama-specific TracedReAct strategy.
"""
...
from bmw_agents.core.prompt_strategies.ollama_traced_react import OllamaTracedReAct
...
async def main() -> None:
    """Run an improved TracedReAct example with Ollama-specific strategy."""
    ...
    # Create the Ollama-specific TracedReAct prompt strategy
    prompt_strategy = OllamaTracedReAct(llm_provider=llm_provider, tools=tools, max_iterations=5)

    # Check initial state
    print(
        f"Initial - Thoughts: {len(prompt_strategy.thoughts)}, Actions: {len(prompt_strategy.actions)}, Observations: {len(prompt_strategy.observations)}"
    )
    ...
    # Check final state
    print(
        f"After run - Thoughts: {len(prompt_strategy.thoughts)}, Actions: {len(prompt_strategy.actions)}, Observations: {len(prompt_strategy.observations)}"
    )
