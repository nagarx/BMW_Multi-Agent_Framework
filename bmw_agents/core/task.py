...
class Task:
    ...
    def is_ready(self) -> bool:
        ...
        return (
            ...
            len(self.dependencies) == len(self.dependency_results)
        )
    ...
    def add_dependency_result(self, task_id: str, result: Any) -> None:
        ...
        if task_id in self.dependencies:
            ...
            logger.warning(f"Attempted to add result for non-dependency {task_id} to task {self.id}")
...
class TaskQueue:
    ...
    def _propagate_result(self, task_id: str, result: Any) -> None:
        """
        Propagate the result of a task to all dependent tasks.
    ...
    def is_complete(self) -> bool:
        ...
        """
        return (
            len(self.completed_tasks) + len(self.failed_tasks) == len(self.tasks) and
            len(self.tasks) > 0
        )
