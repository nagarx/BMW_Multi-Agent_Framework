...
from bmw_agents.core.prompt_strategies.single_response_traced_plan_react import (
    SingleResponseTracedPlanReAct,
)
...
class OllamaSingleResponsePlanReAct(SingleResponseTracedPlanReAct):
    ...
    def __init__(
        ...
        super().__init__(llm_provider, tools, template_path, termination_sequence)
    ...
    async def execute(self, instruction: str, **kwargs: Any) -> str:
        ...
        instruction includes explicit reminders about the expected format
        (Plan/Thought/Action/Observation sequence), helping the model generate responses that can be
        properly parsed. The key difference from the regular SingleResponseTracedReAct is the
        inclusion of an explicit planning step at the beginning.
        ...
        enhanced_instruction = (
            ...
            f"'Plan:', then for each step provide your reasoning as 'Thought:', specify your "
            f"action as 'Action:' with valid JSON, followed by 'Observation:' with the result. "
            f"Repeat this format for each step until you have enough information to provide the "
            f"final answer."
        )

        logger.debug("Executing SingleResponseTracedPlanReAct with Ollama-enhanced instruction")
