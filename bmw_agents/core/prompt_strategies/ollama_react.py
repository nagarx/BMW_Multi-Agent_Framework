...
class OllamaReActPromptStrategy(ReActPromptStrategy):
    ...
    async def execute(self, instruction: str, **kwargs) -> str:
        ...
        logger.debug(f"Before execute - Thoughts: {len(self.thoughts)}, Actions: {len(self.actions)}, Observations: {len(self.observations)}")
        result = await super().execute(enhanced_instruction, **kwargs)
        logger.debug(f"After execute - Thoughts: {len(self.thoughts)}, Actions: {len(self.actions)}, Observations: {len(self.observations)}")
