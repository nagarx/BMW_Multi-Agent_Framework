"""
Ollama-specific SingleResponseTracedReAct prompt strategy for the BMW Agents framework.
This module implements a version of TracedReAct optimized for Ollama models that generate
a complete execution trace in a single response.
"""
...
class OllamaSingleResponseReAct(SingleResponseTracedReAct):
    """
    Implementation of the SingleResponseTracedReAct strategy optimized for Ollama models.
    
    This strategy adapts the SingleResponseTracedReAct strategy to handle Ollama's specific output format
    ...
    def __init__(self, 
        ...
        """
        Initialize an Ollama-specific SingleResponseTracedReAct strategy.
        ...
        if not isinstance(llm_provider, OllamaProvider):
            logger.warning(
                f"OllamaSingleResponseReAct is optimized for OllamaProvider, "
    ...
    async def execute(self, instruction: str, **kwargs) -> str:
        ...
        includes explicit reminders about the expected format (Thought/Action/Observation sequence),
        helping the model generate responses that can be properly parsed.
        ...
        logger.debug(f"Executing SingleResponseTracedReAct with Ollama-enhanced instruction")
